...
module.exports = class ChartOrder {
    ...
    render(){
        ...
        if(this.order.isOpen()){
            ...
            const value_string = via.fn.number.formatPrice(value, this.order.market);
            ...
            if(limit){
                ...
                const limit_string = via.fn.number.formatPrice(this.order.limit, this.order.market);
                ...
                this.flag.attr('transform', `translate(0, ${limitY - Math.floor(FLAG_HEIGHT / 2)})`)
                    .select('text')
                        .attr('x', limit_string.length * 3 + 6)
                        .text(limit_string);

                this.flag.select('rect')
                    .attr('width', limit_string.length * 6 + 12);
            ...
            if(stop){
                ...
                const stop_string = via.fn.number.formatPrice(this.order.stop, this.order.market);
                ...
                this.stopFlag.attr('transform', `translate(0, ${stopY - Math.floor(FLAG_HEIGHT / 2)})`)
                    .select('text')
                        .attr('x', stop_string.length * 3 + 6)
                        .text(stop_string);

                this.stopFlag.select('rect')
                    .attr('width', stop_string.length * 6 + 12);
            }
        }else{
            this.hide();
        }
