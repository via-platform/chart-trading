...
const ESTIMATE = 140;
const CANCEL = 19;
const CROSS = 'M11,.44,6.8,4.68,2.56.44A1.5,1.5,0,0,0,.44,2.56L4.68,6.8.44,11a1.5,1.5,0,1,0,2.12,2.12L6.8,8.92,11,13.17A1.5,1.5,0,1,0,13.17,11L8.92,6.8l4.24-4.24A1.5,1.5,0,1,0,11,.44Z';

const OPEN = ['open', 'canceling'];
const PENDING = ['pending', 'transmitting'];
...
class ChartOrder {
    ...
    draw(){
        ...
        }else if(PENDING.includes(this.order.status) && this.order.isValidOrder()){
        ...
        // if(this.order.limit){
        //     const limit = Math.round(this.panel.scale(this.order.limit));
        //     const receive = (this.order.amount * this.order.limit).toFixed(this.order.market.precision.price);
        //
        //     this.flag.classed('hide', false)
        //         .attr('transform', `translate(0, ${limit - Math.floor(FLAG_HEIGHT / 2)})`)
        //         .select('text')
        //             .text(this.order.limit.toFixed(this.chart.precision));
        //
        //     this.line.classed('hide', false).attr('transform', `translate(0, ${limit - 0.5})`);
        //
        //     if(this.order.status === 'open'){
        //         this.base.text(`${side.toUpperCase()} ${type.toUpperCase()}`);
        //         this.estimate.text(`${amount} ${base}`);
        //     }else{
        //         this.base.text(`(Limit) ${side} ${amount} ${base}`);
        //         this.estimate.text(`For ${receive} ${quote}`);
        //     }
        //
        //     this.tools.classed('hide', false).style('top', `${limit - 10}px`).classed('open', this.order.status === 'open');
        // }
        //
        // if(this.order.type === 'stop'){
        //     if(this.order.status === 'open'){
        //         this.stop.classed('hide', true);
        //         this.stopFlag.classed('hide', true);
        //         this.stopLine.classed('hide', true);
        ...
        //     }
        //
        //     const stop = Math.round(this.panel.scale(this.order.stop));
        //
        //     this.stopFlag.classed('hide', false)
        //         .attr('transform', `translate(0, ${stop - Math.floor(FLAG_HEIGHT / 2)})`)
        //         .select('text')
        //             .text(this.order.stop.toFixed(this.chart.precision));
        //
        //     this.stopLine.classed('hide', false).attr('transform', `translate(0, ${stop - 0.5})`);
        //     this.stopBase.text('Stop Price');
        //     this.stop.classed('hide', false).style('top', `${stop - 10}px`);
        // }
    }
